document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dynamic Resizing Logic for All Screens
    function resizeBook() {
        ...
        // Calculate available space securely
        // Header height ~90px, controls ~80px, plus padding
        const availableHeight = windowHeight - 190; 
        const availableWidth = windowWidth - 30; // 15px padding on each side
        
        // Base dimensions of the open 3D book spread (450 * 2 + some buffer)
        const baseWidth = 920; 
        ...
        // Restrict scale limits
        if (scale > 1.2) scale = 1.2;
        if (scale < 0.2) scale = 0.2; // absolute minimum safeguard
        
        workspace.style.transform = `scale(${scale})`;
    }

    // Generate Dynamic Om Background
    function generateOmBackground() {
        const container = document.getElementById('om-background');
        if (!container) return;

        const numOms = 35; // Number of symbols

        for (let i = 0; i < numOms; i++) {
            const om = document.createElement('div');
            om.className = 'floating-om';
            om.innerText = 'ॐ';

            // Randomize properties for an organic feel
            const size = Math.random() * 3 + 1; // 1rem to 4rem sizes
            const leftPos = Math.random() * 100; // 0% to 100% width
            const animDuration = Math.random() * 30 + 15; // 15s to 45s duration
            const animDelay = Math.random() * -45; // Start at different times
            const maxOpacity = Math.random() * 0.15 + 0.05; // 0.05 to 0.20 opacity

            om.style.fontSize = `${size}rem`;
            om.style.left = `${leftPos}%`;
            om.style.animationDuration = `${animDuration}s`;
            om.style.animationDelay = `${animDelay}s`;
            om.style.setProperty('--max-opacity', maxOpacity);
